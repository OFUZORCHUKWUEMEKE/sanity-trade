...
describe("PaperTrader.tryEnter", () => {
  it("does not enter below the score threshold", () => {
    const { paperTrader, insertOne } = makeTrader();
    paperTrader.tryEnter(mint, deployer, 65, false, 0.0001, 10, now);
  ...
  it("does not enter a hard-rejected token even above threshold", () => {
    const { paperTrader, insertOne } = makeTrader();
    paperTrader.tryEnter(mint, deployer, 95, true, 0.0001, 10, now);
    paperTrader.onTrade(trade({ side: "sell", trader: deployer, priceSol: 0.0001 }));
    expect(insertOne).not.toHaveBeenCalled();
  });

  it("does not enter without price data", () => {
    const { paperTrader, insertOne } = makeTrader();
    paperTrader.tryEnter(mint, deployer, 95, false, undefined, 10, now);
    paperTrader.onTrade(trade({ side: "sell", trader: deployer, priceSol: 0.0001 }));
    expect(insertOne).not.toHaveBeenCalled();
  });
});
...
describe("PaperTrader mark-to-market and exits", () => {
  it("persists a partial take-profit fill with a positive pnl", async () => {
    const { paperTrader, insertOne } = makeTrader();
    paperTrader.tryEnter(mint, deployer, 90, false, 0.0001, 10, now);
  ...
  it("frees a concurrency slot once a position fully closes", () => {
